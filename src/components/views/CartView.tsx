...
export const CartView: React.FC = () => {
  ...
  return (
    <div className="py-2 space-y-5 pb-36 md:pb-12 animate-fadeIn">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2.5 sm:mt-3 pt-1">
                      {/* Quantity Stepper */}
                      {(() => {
                        const maxStock = Math.min(10, item.product.stockCount || 10);
                        return (
                          <div className="inline-flex items-center border border-gray-200 rounded-xl bg-gray-50/70 p-0.5 shadow-2xs">
                            ...
                            <button
                              onClick={() => updateCartQuantity(item.productId, Math.min(maxStock, item.quantity + 1))}
                              disabled={item.quantity >= maxStock}
                              className="w-7 h-7 flex items-center justify-center text-gray-600 hover:text-black disabled:opacity-30 rounded-lg hover:bg-white active:scale-95 transition-all"
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-500">Tap to uncover a mystery coupon</p>
                </div>
              </div>
            ...
            {appliedCoupon ? (
              ...
              <div className="space-y-2">
                <form onSubmit={handleApplyCoupon} className="flex gap-2">
                  <input
                    type="text"
                    placeholder="e.g. SBSWELCOME"
                    value={couponCodeInput}
                    onChange={(e) => setCouponCodeInput(e.target.value.toUpperCase())}
                    className="flex-1 border border-gray-200 rounded-xl px-3 py-2 text-xs uppercase font-bold tracking-wider outline-none focus:border-[#F95721] focus:ring-1 focus:ring-orange-100"
                  />
                  <button
                    type="submit"
                    className="px-5 py-2 bg-[#F95721] hover:bg-[#E44813] text-white text-xs font-black rounded-xl active:scale-95 transition-all shadow-xs"
                  >
                    Apply
                  </button>
                </form>
              ...
              {appliedCoupon && couponDiscount > 0 && (
                <div className="flex justify-between items-center text-emerald-700 bg-emerald-50 px-2.5 py-1.5 rounded-xl border border-emerald-100 font-semibold">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[11px] font-black uppercase px-1.5 py-0.5 bg-emerald-600 text-white rounded-md">
                      {appliedCoupon.code}
                    </span>
      ...
      <div className="md:hidden fixed bottom-[calc(60px+var(--safe-bottom))] left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-gray-100 px-4 py-2.5 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] sticky-gpu">
        <div className="max-w-md mx-auto flex items-center justify-between gap-3">
          <div>
            <span className="text-[11px] uppercase font-bold text-gray-400 tracking-wider block">Total Amount</span>
