    ...
    <div className="space-y-4 pb-28 animate-fadeIn">
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by title, category, or barcode/ID..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-white border border-gray-200 rounded-2xl pl-9 pr-10 py-2.5 text-xs font-medium outline-none focus:border-[#F95721] shadow-2xs"
          />
          {searchQuery && (
            <button
              type="button"
        ...
        <button
          ...
          className="px-3.5 py-2.5 bg-gray-900 hover:bg-black text-white rounded-2xl text-xs font-bold flex items-center gap-1.5 shadow-2xs transition-all active:scale-95 shrink-0"
          title="Scan product barcode to search"
        >
              ...
              >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    ...
                    {/* Barcode or Custom ID Indicator */}
                    {(p.barcode || (!p.id.startsWith('p_') && !p.id.startsWith('prod_'))) && (
                      <div className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-gray-100 text-gray-600 text-[10px] font-mono font-semibold max-w-fit">
                        <Barcode className="w-3 h-3 text-gray-400 shrink-0" />
                        <span className="truncate">{p.barcode || p.id}</span>
                      </div>
                    )}
      ...
      {/* Barcode Scanner Modal for Quick Lookup */}
      <BarcodeScannerModal
        ...
        onScanSuccess={(code) => {
          ...
          if (found) {
            ...
            showToast(`Scanned "${code}". Filtered catalog.`);
          }
        }}
      />
    </div>
