...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onSelect }) => {
  const { addToCart, toggleWishlist, isInWishlist, setSelectedProductDetail } = useStore();
    ...
    >
      ...
      <div className="mt-2 flex-1 flex flex-col justify-between">
        <div onClick={handleClick} className="cursor-pointer space-y-1">
          <h3 className="font-bold text-gray-900 text-xs sm:text-sm line-clamp-2 leading-snug group-hover:text-[#F35C16] transition-colors min-h-[32px] sm:min-h-[38px]">
            {product.name}
          </h3>
          ...
          <div className="flex items-baseline flex-wrap gap-1.5 pt-0.5">
            ...
            <span className="text-[10px] font-extrabold text-[#F35C16] bg-orange-50 px-1.5 py-0.5 rounded">
        ...
        <button
          ...
          className="mt-2.5 w-full h-9 sm:h-10 px-3 bg-[#F35C16] hover:bg-[#e04a08] text-white text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 shadow-xs active:scale-95 transition-all"
        >
